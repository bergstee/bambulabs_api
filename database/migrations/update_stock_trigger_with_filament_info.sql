-- Update the stock transaction trigger to include filament information
-- This will add color and filament details to the transaction notes
    ...
    IF NEW.status = 'FINISH' AND NEW.end_time IS NOT NULL AND OLD.end_time IS NULL THEN
        ...
        LOOP
            IF item_record.item_id IS NOT NULL AND item_record.quantity IS NOT NULL THEN
                RAISE NOTICE '  Inserting stock for item_id: %, quantity: %, job_id: %', item_record.item_id, item_record.quantity, NEW.id;

                -- Insert into stock_transactions with enhanced notes
                INSERT INTO stock_transactions
                    (item_id, quantity, transaction_type, transaction_date, notes, print_job_id, item_assembly_id)
                VALUES
                    (item_record.item_id, item_record.quantity, 'PRINT_COMPLETE', NEW.end_time, notes_text, NEW.id, item_record.item_subassembly_id);
            ...
            ELSE
                 RAISE WARNING '  Skipping stock record due to NULL item_id (%) or quantity (%) associated with filename: %', item_record.item_id, item_record.quantity, NEW.filename;
            END IF;
        END LOOP;

        -- If no items were found for the filename, log a warning
        IF NOT FOUND THEN
             RAISE WARNING 'No associated item IDs or quantities found in printer_file_models for filename: %. Cannot record stock.', NEW.filename;
        END IF;
...
'Trigger function that creates stock transactions when a print job finishes. Enhanced to include filament color and details from printer_job_filaments table in transaction notes.';
